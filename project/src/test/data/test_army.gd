...
func test_get_total_attack_overflow() -> void:
	...
	PlayerData.army.gobs[0].count = Big.new(999_999_999_999_999_999)
	PlayerData.army.gobs[1].count = Big.new(999_999_999_999_999_999)
...
func test_from_json_dict() -> void:
	...
	assert_eq(1, PlayerData.army.gobs.size())
	assert_eq("fire4", PlayerData.army.gobs[0].name)
	assert_eq(2, PlayerData.army.gobs[0].count.to_int())
	assert_eq(4, PlayerData.army.gobs[0].level)
	assert_eq(Gobs.Type.FIRE, PlayerData.army.gobs[0].type)
	assert_eq(14, PlayerData.army.gobs[0].hp)
	assert_eq(14, PlayerData.army.gobs[0].hp_max)
	assert_eq(12, PlayerData.army.gobs[0].attack)
	assert_eq(55, PlayerData.army.gobs[0].gold)
	assert_eq(3, PlayerData.army.gobs[0].xp)
...
func test_to_glob() -> void:
	...
	assert_eq(3, PlayerData.army.gobs.size())
	assert_eq("fire4", PlayerData.army.gobs[0].name)
	assert_eq("fire5", PlayerData.army.gobs[1].name)
	assert_eq("water6", PlayerData.army.gobs[2].name)
