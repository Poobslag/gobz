...
## Returns the [member InputEventKey.keycode] for a key release event, or -1 if the event is not a key release event.
static func key_release(event: InputEvent) -> int:
	var keycode := -1
