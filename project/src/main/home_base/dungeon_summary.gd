...
var _bar_widths: Array[float] = [50, 50, 50, 50, 50]
var _bar_types: Array[Gobs.Type] = [Gobs.FIRE, Gobs.WATER, Gobs.GRASS, Gobs.ANGEL, Gobs.DEVIL]

func _recalculate_layout() -> void:
	var dungeon_select_info: Dictionary[Variant, Variant] = Dungeons.get_dungeon_select_info(dungeon)
	...
	var bar_width: float = remap(dungeon.recon_army.get_total_goblins().to_float(), 0, boss_dungeon_goblins, 100, 440)
	
	_bar_types.clear()
	_bar_widths.clear()
	for summary: Dictionary[Variant, Variant] in dungeon_select_info["type_summaries"]:
		_bar_types.append(summary["type"])
		_bar_widths.append( \
				bar_width * summary["goblins"].to_float() / dungeon.recon_army.get_total_goblins().to_float())
